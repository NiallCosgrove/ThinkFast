...
TOKENIZER_CONFIG_PATH = "qwen2.5-1.5b-reasoning-template-config.json"  #1.5b version
#TOKENIZER_CONFIG_PATH = "qwen2.5-reasoning-template-config.json"      #7b version
...
def setup_model(model_name):
    ...
    if not os.path.exists(SAVE_PATH):
        ...
        base_path = os.path.join(SAVE_PATH, f"models--{model_name.replace('/', '--')}")
    ...
    if "Qwen2.5" in model_name:
        ...
        print(f"Warning: Model {model_name} is not Qwen2.5. Skipping tokenizer modification.")
...
def load_model_and_tokenizer(model_name,max_seq_length,lora_rank, resume_from_checkpoint=False):
    model_dir = setup_model(model_name)
    print(f"{model_dir} should now be present and ready for fine tuning")
...
def main():
    ...
    parser.add_argument("--lora_rank", type=int, default=16, help="Higher is smarter, but slower 8,16,32,64...")
